...
# Define the WinProbabilityModel class
class WinProbabilityModel:
    def __init__(self):
        pass

    def predict(self, overs, runs, wickets):
        return max(0, min(1, 1 - (wickets/10) + (runs/200) - (overs/20)))

# Get the win probability model
win_prob_model_filename = 'win_probability_model.pkl'
with open(win_prob_model_filename, 'rb') as f:
...
if st.button('Predict Score'):
    ...
    st.success(x)
    # Calculate win probabilities for the overs
    win_probs = []
    for over in range(1, 21):
        win_prob = win_prob_model.predict(over, runs, wickets)
        win_probs.append(win_prob)

    # Plot the win probabilities
    plt.figure(figsize=(10, 6))
    sns.lineplot(x=range(1, 21), y=win_probs, marker='o')
    plt.title('Win Probability Over the Overs')
    plt.xlabel('Overs')
    ...
    # Pie chart for winning percentage
    win_prob_batting_team = win_prob_model.predict(overs, runs, wickets)
    win_prob_bowling_team = 1 - win_prob_batting_team  # Assuming the rest of the probability goes to the bowling team

    labels = [batting_team, bowling_team]
    sizes = [win_prob_batting_team, win_prob_bowling_team]
